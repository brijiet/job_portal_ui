...
interface AllJobsState {
  ...
  error: boolean;
  success: boolean;
  allApplicant: Array<AllApplicant>;
  department: any;
  location: any;
  workMode: any;
  roleCategory: any;
...
const getFilterApplicantSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getFilterApplicant.fulfilled, (state, action: PayloadAction<Array<AllApplicant>>) => {
      state.loading = false;
      state.success = true;
      state.error = false;
...
export const { clearGetFilterJobsSlice,
  ...
  setSearchLocation,
  setSearchKeySkills,
  setToggleFilter } = getFilterApplicantSlice.actions;