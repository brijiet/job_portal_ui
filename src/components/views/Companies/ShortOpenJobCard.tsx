...
const ShortOpenJobCard = ({ openJob, onClickJobCard, setSaveJobCount, jobDetail }: any) => {
  ...
  useEffect(() => {
    getCheckJobToSave(Number(userId), Number(openJob?.id)).then((count: any) => {
      if (count > 0) setSaveSimilarJobCount(true);
    });
  }, [openJob]);

  useEffect(() => {
    const parsedDate = parseISO(openJob?.createdAt);
  ...
  return (
    <div className=" p-5 bg-white rounded-xl flex-col justify-start items-start inline-flex w-full border border-[#E0E7FF] cursor-pointer" onClick={() => onClickJobCard(openJob.id)}>
      <div className="self-stretch  flex-col justify-start items-start gap-3 flex ">
        <div className="self-stretch  flex-col justify-start items-start gap-1 flex ">
          <div className="flex justify-between w-full ">
            ...
            <button className='p-2' onClick={() => setButtonClick("Save", openJob?.id)}>
              {saveSimilarJobCount ? (
          ...
          <div className="justify-start items-center gap-3 ">
            <span className="text-slate-500 text-sm font-normal leading-snug tracking-tight">
              {openJob?.aboutCompany}
            </span>
          </div>
        </div>
        <div className="justify-start items-center gap-1 inline-flex">
        {openJob?.workMode?.title &&
...
export default ShortOpenJobCard;
