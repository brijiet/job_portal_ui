...
import { setSalarys, setMaxSalaryId } from '../../../../store/reducers/applicant/GetFilterApplicant';
import { ChevronUpIcon } from '@heroicons/react/20/solid';
import Select from 'react-select';

export const SalaryApplicantBasedFilter = ({ handleSalaryFilter, isOpen }: any) => {
  const dispatch = useAppDispatch();
  const { filtersData, salary: intialSalary } = useAppSelector((state) => state.getFilterApplicant);
...
const FiltersApplicantSalary = () => {
  const dispatch = useAppDispatch();
  const { salary: intialSalary, maxSalaryId } = useAppSelector((state) => state.getFilterApplicant);
...
export default FiltersApplicantSalary;