...
const UpdateEmailTemplate = ({
  ...
  const onSubmit = (submitData: any) => {
    setData(submitData);
    setmailTemplateHeading("Preview Mail Template");
  };
  useEffect(() => {
    (async () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      data?.subject && setValue("mailSubject", data?.subject);
      ...
      data?.fromWorkExperience &&
        setValue("fromWorkExperience", {
          value: data?.fromWorkExperience?.id,
          label: data?.fromWorkExperience?.title,
        });
      data?.toWorkExperience &&
        setValue("toWorkExperience", {
          value: data?.toWorkExperience?.id,
          label: data?.toWorkExperience?.title,
        });
      data?.currency &&
        setValue("currency", {
      ...
      data?.minSalaryRange &&
        setValue("fromSalaryRange", {
          value: data?.minSalaryRange?.id,
          label: data?.minSalaryRange?.title,
        });
      data?.maxSalaryRange &&
        setValue("toSalaryRange", {
          value: data?.maxSalaryRange?.id,
          label: data?.maxSalaryRange?.title,
        });
      data?.mailTemplateNumberSystem &&
        setValue("numberSystem", {
          value: data?.mailTemplateNumberSystem?.id,
          label: data?.mailTemplateNumberSystem?.title,
        });
      data?.mailTemplateJobLocation &&
        data.mailTemplateJobLocation.map((location: any) => {
          location &&
            selectedJobsLocation.push({
              value: location?.templateLocation?.id,
              label: location?.templateLocation?.title,
            });
        });
      data?.mailTemplateJobLocation &&
        setValue("jobLocation", selectedJobsLocation);
      data?.mailTemplateKeySkills &&
        data?.mailTemplateKeySkills.map((skill: any) => {
          skill &&
            selectedKeySkills.push({
              value: skill?.keySkills?.id,
              label: skill?.keySkills?.title,
            });
        });
      data?.mailTemplateKeySkills && setValue("keySkills", selectedKeySkills);
    }
  }, [data]);
