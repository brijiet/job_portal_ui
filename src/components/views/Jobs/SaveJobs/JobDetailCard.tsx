...
import { getCheckJobApplicant } from '../../../utils/utils';
import NoRecords from '../../../commonComponents/NoRecords';

const JobDetailCard = ({ setApplyJob, setDeleteSave, inbox, jobId, setJobId, jobCard, loading }: any) => {
  ...
  const onSubmit = (data: IFormApplyJobsWithoutQuestionnaire | IFormSaveJobs) => {
    ...
    if (buttonClick === 'Delete') {
      dispatch(deleteSaveJob({
        "user": userId && parseInt(userId),
        "jobs": jobId && parseInt(jobId)
      })).then((data: any) => {
        if (data?.payload?.affected === 1) {
          setDeleteSave(true);
          ...
          <div className="border border-[#E0E7FF] rounded-xl p-5 bg-white">
            <div className="self-stretch h-44 flex-col justify-start items-start gap-5 flex">
              <div className="self-stretch justify-start items-center gap-3 inline-flex">
                <img className="w-14 h-14 rounded-lg" src={`${process.env.REACT_APP_IMAGE_BASE_URL}company_logo.png`} alt="companyLogo" />
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="justify-start items-center gap-5 inline-flex mt-4">
                ...
                {applicantCount && inbox === 'Save' && <button className="w-48  px-6 py-1.5 bg-green-600 rounded-lg shadow justify-center items-center gap-3 flex" >
                  <span className="text-white text-xl font-medium  leading-normal tracking-tight">Applied</span>
                </button>}
                {inbox === 'Save' && <button className="w-28 pl-6 pr-3 py-1.5 bg-red-300 rounded-lg justify-center items-center  gap-3 flex">
