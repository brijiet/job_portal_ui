...
import {
  IFormApplyJobsWithoutQuestionnaire,
  IFormSaveJobs,
} from "../../../interface/jobSeeker/applyJobs";
...
const JobDescription = () => {
  ...
  const onSubmit = (
    ...
    if (buttonClick === "Save") {
      setSaveJobCount(prevState => true);
      dispatch(
        saveJobs({
          jobSeekerProfile: userId && parseInt(userId),
          jobs: id && parseInt(id),
  ...
  useEffect(() => {
    getCheckJobApplicant(Number(userId), Number(id)).then((count: any) => {
      if (count > 0) setApplicantCount(true);
    });

    getCheckJobToSave(Number(userId), Number(id)).then((count: any) => {
      if (count > 0) setSaveJobCount(true);
    });
  }, [id, userId, checkEmpty]);
  ...
  const handleLoginToApply = () => {
    const params = btoa(`/allJobs/jobDescription/${id}`);
    navigate(`/login/${params}`);
  };
  const handleRegisterToApply = () => {
    const params = btoa(`/allJobs/jobDescription/${id}`);
    ...
    <Fragment>
      ...
      <div className="grid grid-cols-12 gap-10 py-6 px-32 bg-[#F8FAFC] ">
          ...
          <div className="col-start-1 col-end-8  p-5">
              ...
              <div className="self-stretch h-44 flex-col justify-start items-start gap-5 flex">
                <div className="self-stretch justify-start items-center gap-3 inline-flex">
                  <img
                    ...
                    src={jobDetail?.company?.companyImage ? `${process.env.REACT_APP_COMPANY_PICTURE_FILE_LOCATION}/${jobDetail?.company?.companyImage}`.replace(/"/g, '') as any : `${process.env.REACT_APP_IMAGE_BASE_URL}companyLogoDefault.svg`}
                    alt="companyLogo"
                  />
                  <div className="grow shrink basis-0 flex-col justify-start items-start gap-1 inline-flex">
                    <div className="self-stretch text-slate-900 text-2xl font-bold  leading-7 tracking-tight">
                      {jobDetail?.title}
                    </div>
                  ...
                  <div className="self-stretch justify-start items-start gap-5 inline-flex">
                    <div className="justify-start items-center gap-2 flex">
                      ...
                      <span className="text-slate-500 text-base font-medium leading-snug tracking-tight">
                        {removeYearText(jobDetail?.totalExpYearStart?.title)} -
                      </span>
                      <span className="text-slate-500 text-base font-medium leading-snug tracking-tight">
                        {jobDetail?.totalExpYearEnd?.title}
                      </span>
                    </div>
                    {
                      !(jobDetail?.hideSalaryDetails) ?
                        <div className="justify-start items-center gap-2 flex">
                          <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}MoneyIcon.svg`} alt="moneyIcon" />
                          <span className="text-slate-500 text-base font-medium leading-snug tracking-tight">
                            {jobDetail?.currency?.title}
                            {jobDetail?.payScaleLowerRange?.title} -
                          </span>
                          {jobDetail?.payScaleUpperRange && (
                            <span className=" text-slate-500 text-base font-medium leading-snug tracking-tight">
                              {" "}
                              {jobDetail?.payScaleUpperRange?.title}{" "}
                              {jobDetail?.numberSystem?.title}{" "}
                              {jobDetail?.recurrence?.title}
                            </span>
                          )}
                        </div> : <div className="justify-start items-center gap-2 flex">
                          <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}MoneyIcon.svg`} alt="moneyIcon" />
                          <span className="text-slate-500 text-base font-medium leading-snug tracking-tight">Not Disclosed</span>
                        </div>
                    }
                    <div className="justify-start items-center gap-2 flex">
                      <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}LocationIcon.svg`} alt="location" />
                      {jobDetail?.jobsLocation?.map((loc: any, index) => (
                        <span key={index} className="text-slate-500 text-base font-medium leading-snug tracking-tight">
                          {(index ? ', ' : '') + loc?.location?.title}
                        </span>
                      ))}
                    </div>
                  </div>
                  <div className="justify-start items-center gap-5 inline-flex">
                    {jobDetail?.workMode?.title && (
                    ...
                    {jobDetail?.employmentType?.title && (
                ...
                <div className="justify-start items-center gap-5 inline-flex mt-4">
                  {Cookies.get("token") ? (
                    <>
                      ...
                      {applicantCount && (
                        <button className="w-48  px-6 py-1.5 bg-green-600 rounded-lg shadow justify-center items-center gap-3 flex">
                          <span className="text-white text-base font-medium  leading-normal tracking-tight">
                            Applied
                          </span>
                        </button>
                      )}
                      <button className="w-28 pl-6 pr-3 py-1.5 bg-indigo-50 rounded-lg justify-center items-center  gap-3 flex" onClick={() => setButtonClick("Save")}>
                        <button
                          className="text-indigo-900 text-base font-medium  leading-normal tracking-tight flex "
                        >
                          Save
                    ...
                    <>
                      ...
                      <button
                        className="w-58  px-6 py-1.5 bg-indigo-600 rounded-lg shadow justify-center items-center gap-3 flex"
                        onClick={() => {
                          handleRegisterToApply();
                          setButtonClick("RegisterToApply");
                        }}>
                        <span className="text-white text-base font-medium  leading-normal tracking-tight">
                          Register to Apply
                        </span>
            ...
            <div className="border border-[#E0E7FF] rounded-xl p-5 mt-4 bg-white ">
              <div className="  flex-col justify-start items-start gap-5 flex-wrap">
                <div className=" text-slate-900 text-base font-bold leading-snug tracking-tight">
                  Job Description
                </div>
              ...
              <div className="self-stretch  flex-col justify-start items-start gap-5 flex ">
                ...
                <div className=" justify-start items-start gap-3 flex-row flex flex-wrap">
                  {jobDetail?.jobsKeySkills?.map((keySkill) => (
                    <div key={keySkill?.id} className=" px-3 py-2 bg-slate-50 rounded-lg justify-center items-center gap-2.5 flex">
                      <span className="text-black text-base font-normal  leading-snug tracking-tight">
                        {keySkill?.keySkills?.title}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="border border-[#E0E7FF] rounded-xl p-5 mt-4 bg-white">
              <span className="self-stretch text-slate-900 text-base font-bold leading-snug tracking-tight">
                About the company
              </span>
              <div className="self-stretch justify-start items-center gap-3 flex mt-2">
                <img
                  className="w-14 h-14 rounded-lg"
                  src={jobDetail?.company?.companyImage ? `${process.env.REACT_APP_COMPANY_PICTURE_FILE_LOCATION}/${jobDetail?.company?.companyImage}` : `${process.env.REACT_APP_IMAGE_BASE_URL}companyLogoDefault.svg`}
                  alt="companyLogo"
                />
                <div className="grow shrink basis-0 flex-col justify-start items-start gap-1 inline-flex">
                  <div className="self-stretch text-slate-900 text-2xl font-bold leading-7 tracking-tight overflow-hidden whitespace-nowrap text-ellipsis w-96">
                    {jobDetail?.company?.title}
                  </div>
                  <div className="w-40 justify-start items-center gap-2 inline-flex">
                    <div className="justify-start items-center gap-1 flex">
                      <AiFillStar color="yellow" />
                      {jobDetail?.company?.rating ? (
                        <div className="text-black text-sm font-normal leading-none tracking-tight">
                          {jobDetail?.company?.rating}
                        </div>
                      ) : (
                        <div>N.A</div>
                    ...
                    {jobDetail?.company?.reviews && (
                      <Fragment>
                        <div className=" border-l border-indigo-100 h-4"></div>
                        <div className="text-slate-500 text-sm font-normal leading-none tracking-tight">
                          {jobDetail?.company?.reviews}
                        </div>
                      </Fragment>
                    )}
              ...
              <div className="justify-start items-start gap-5 inline-flex mt-2">
                <div className="justify-start items-center gap-2 flex">
                  <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}LocationIcon.svg`} alt="location" />
                  {jobDetail?.company?.location?.length > 0 ? (
                    jobDetail?.company?.location?.map(({ loc, index }: any) =>
                      index < locationCount ? (
                        <span className="ml-1 text-slate-500 text-base font-medium">
                          {loc?.title},{" "}
                        </span>
                      ) : (
              ...
              <div className="px-2 py-1.5  w-44 bg-indigo-50 rounded-lg justify-center items-center text-center mt-2">
                <button className="text-indigo-900 text-base font-medium  leading-snug tracking-tight">
                  More open positions
                </button>
        ...
        <div className="col-start-8 col-end-13">
          ...
          <SimilarJobs
            keySkillParams={jobDetail?.jobsKeySkills}
